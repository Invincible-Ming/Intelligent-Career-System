...
class MilvusService:
    ...
    def client(self):
        if self._client is None:
            self._client = MilvusClient(**self._client_options)
        return self._client
    ...
    async def insert(
        ...
        for content, vector in zip(
            ...
            if len(content.encode("utf-8")) > 8192:
                raise ValueError("文本块超过 Milvus content 的 8192 字节限制，请降低 CHUNK_SIZE")
    ...
    async def search(
        ...
        if allowed_document_ids is not None:
            if not allowed_document_ids:
                return []
            for identifier in allowed_document_ids:
                validate_document_id(identifier)
            owned_filter = "document_id in " + json.dumps(allowed_document_ids)
            filter_expression = f"({filter_expression}) and ({owned_filter})" if filter_expression else owned_filter
