...
class RerankerService:
    ...
    def _rerank_sync(
        ...
        pairs, owners = make_rerank_pairs(
            self.model.tokenizer, query,
            [str(result.get("content", "")) for result in results],
            settings.RERANKER_MAX_LENGTH,
        )
        logger.info("重排候选=%s，token窗口=%s", len(results), len(pairs))
        ...
        # A relevant tail window must not be lost to head-only truncation.
        aggregated = [float("-inf")] * len(results)
        for owner, score in zip(owners, scores, strict=True):
            aggregated[owner] = max(aggregated[owner], float(score))
