...
calls = []
options = {'registration_enabled': True, 'chat_max_input_chars': 4000, 'chat_max_output_tokens': 2048,
           'chat_total_timeout': 90,
           'analysis_total_timeout': 180, 'max_upload_size_mb': 20, 'search_max_input_chars': 2000,
           'jd_max_input_chars': 30000, 'feedback_max_input_chars': 2000}
...
def api_request(method, url, **kwargs):
    ...
    if url.endswith('/api/auth/login'): return response(
        {'access_token': 'test-session-token', 'user': {'username': 'alice', 'is_admin': False}})
...
class FrontendAuthTests(unittest.TestCase):
    ...
    def test_login_guard_and_logout_clear_private_ui(self):
        ...
        with patch('requests.request', side_effect=api_request), \
            ...
            self.assertTrue(any(
                '/api/documents' in url and headers.get('Authorization') == 'Bearer ' + token for _, url, headers in
                calls))
    ...
    def test_invalid_cookie_cannot_render_private_content(self):
        ...
        with patch('requests.request', side_effect=invalid_me), \
                patch.object(type(st.context), 'cookies', new_callable=PropertyMock,
                             return_value={'career_session': 'x' * 43}), \
                patch('streamlit.components.v2.component', return_value=lambda **kwargs: None):
            path = Path(__file__).resolve().parents[2] / 'frontend' / 'app.py'
            app = AppTest.from_file(str(path), default_timeout=15).run()
            self.assertFalse(app.exception)
    ...
    def test_match_progress_and_structured_report_are_rendered_before_review(self):
        report = {'total_score': 78, 'match_level': '较高匹配',
                  'scores': {key: 80 for key in ['skill', 'experience', 'responsibility', 'education', 'bonus']},
        ...
        document = {'id': 'synthetic-resume', 'filename': 'synthetic.txt', 'document_type': 'resume', 'status': 'ready',
                    'chunk_count': 1}
        ...
        events = [('init', {'run_id': 'synthetic-run'}), ('start', {}),
                  ('node_update',
                   {'node': 'resume_agent', 'status': 'completed', 'message': '简历分析完成（复用已有结果）',
                    'cache_hit': True}),
                  ('node_update', {'node': 'jd_agent', 'status': 'completed', 'message': '岗位分析完成（复用已有结果）',
                                   'cache_hit': True}),
