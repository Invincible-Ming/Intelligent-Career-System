...
from app import auth, api, chat_api
...
from app.core.limits import BudgetExceeded, InvocationBudget, SecurityLimitsMiddleware, bound_history, invocation_budget, message_cost, model_request_timeout, operation_timeout
from app.services.milvus_service import MilvusService
from app.core.models import AgentRun, AuthSession, Conversation, Document, DocumentParse, Message, OperationLease, RateLimitBucket
...
class BudgetTests(unittest.IsolatedAsyncioTestCase):
    def test_match_timeout_is_separate_and_model_timeout_respects_deadline(self):
        ...
        finally:invocation_budget.reset(token)
    ...
    async def test_retries_count_and_propagate_budget(self):
        service=BailianService();request=AsyncMock(side_effect=RuntimeError('upstream'))
        service._chat_client=SimpleNamespace(chat=SimpleNamespace(completions=SimpleNamespace(create=request)))
    ...
    async def test_permanent_quota_error_is_not_retried(self):
        service=BailianService();request=AsyncMock(side_effect=ModelServiceError('AllocationQuota.FreeTierOnly',403))
        service._chat_client=SimpleNamespace(chat=SimpleNamespace(completions=SimpleNamespace(create=request)))
    ...
    async def test_stream_cap_and_close(self):
        class Stream:
            closed=False
            def __aiter__(self):return self
            async def __anext__(self):return SimpleNamespace(choices=[SimpleNamespace(delta=SimpleNamespace(content='a'*40))])
            async def close(self):self.closed=True
        stream=Stream();request=AsyncMock(return_value=stream);service=BailianService()
        service._chat_client=SimpleNamespace(chat=SimpleNamespace(completions=SimpleNamespace(create=request)))
        with self.assertRaises(BudgetExceeded):
            async for _ in service.stream_messages(messages=[{'role':'user','content':'q'}],max_output_tokens=2):pass
        self.assertEqual(request.call_args.kwargs['max_tokens'],2);self.assertTrue(stream.closed)
    async def test_milvus_filter_before_search(self):
        service=MilvusService();service.client=MagicMock();service.client.search.return_value=[[]]
        identifier=str(uuid.uuid4())
        await service.search(query_vector=[0.0]*settings.EMBEDDING_DIMENSION,document_type='knowledge',allowed_document_ids=[identifier])
        expression=service.client.search.call_args.kwargs['filter']
        self.assertIn(identifier,expression);self.assertIn('document_type',expression)
        service.client.search.reset_mock()
        self.assertEqual(await service.search(query_vector=[0.0]*settings.EMBEDDING_DIMENSION,allowed_document_ids=[]),[])
    ...
    async def test_bm25_filter_before_ranking(self):
        ...
        self.assertTrue(result);self.assertEqual({row['document_id'] for row in result},{'mine'})
    async def test_parallel_graph_shares_one_budget(self):
        from langgraph.graph import StateGraph, START, END
        from app.core.limits import charge_model_call
        builder=StateGraph(dict)
        async def node(state):charge_model_call();return {}
        builder.add_node('a',node);builder.add_node('b',node)
        builder.add_edge(START,'a');builder.add_edge(START,'b')
        builder.add_edge('a',END);builder.add_edge('b',END)
        token=invocation_budget.set(InvocationBudget(time.monotonic()+10,1))
        try:
            with self.assertRaises(BudgetExceeded):await builder.compile().ainvoke({})
...
class AuthIntegrationTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.patches=[patch.object(auth,'AsyncSessionLocal',self.factory),patch.object(chat_api,'AsyncSessionLocal',self.factory),
                      patch.object(chat_api,'engine',self.engine),patch.object(api,'AsyncSessionLocal',self.factory)]
        for item in self.patches:item.start()
        self.app=FastAPI();self.app.add_middleware(SecurityLimitsMiddleware)
        for router in (auth_router,api.router,chat_api.router,evaluation_router):self.app.include_router(router,prefix='/api')
        async def db():
        ...
        async def validation(request,exc):
            return JSONResponse({'detail':[{'loc':r['loc'],'msg':r['msg'],'type':r['type']} for r in exc.errors()]},status_code=422)
        ...
        for username in ('alice','bob'):
            ...
            self.accounts.append((uuid.UUID(response.json()['user']['id']),{'Authorization':'Bearer '+response.json()['access_token']}))
        self.alice,self.ah=self.accounts[0];self.bob,self.bh=self.accounts[1]
    ...
    async def seed(self):
        async with self.factory() as session:
            da=Document(owner_id=self.alice,filename='alice.txt',document_type='knowledge',minio_object_key='a',status='ready')
            db=Document(owner_id=self.bob,filename='bob.txt',document_type='resume',minio_object_key='b',status='ready')
            ...
            ca=Conversation(owner_id=self.alice,title='a',meta_data={});cb=Conversation(owner_id=self.bob,title='b',meta_data={})
            session.add_all([da,db,run,ownrun,ca,cb]);await session.commit()
    ...
    async def test_login_register_logout_expiry(self):
        ...
        response=await self.client.post('/api/auth/login',json={'username':'alice','password':'wrong-password-long'})
        self.assertEqual(response.status_code,401)
        response=await self.client.post('/api/auth/register',json={'username':'mallory','password':'test-password-long','is_admin':True})
        self.assertEqual(response.status_code,422);self.assertNotIn('test-password-long',response.text)
        ...
        async with self.factory() as session:
            for row in (await session.execute(select(AuthSession))).scalars():row.expires_at=datetime.now(timezone.utc)-timedelta(seconds=1)
    ...
    async def test_browser_cookie_login_restore_logout_and_csrf(self):
        ...
        with patch.object(settings,'AUTH_COOKIE_SECURE',False):
            ...
            bad=await self.client.post(endpoint+'/browser-login',data=form,headers={'Origin':'http://attacker.invalid'})
            self.assertEqual(bad.status_code,403)
            bad=await self.client.post(endpoint+'/browser-login',data={**form,'return_to':'https://attacker.invalid/'},headers=headers)
            self.assertEqual(bad.status_code,400)
            bad=await self.client.post(endpoint+'/browser-login',data={**form,'return_to':'http://127.0.0.1:8501/'},headers=headers)
            self.assertEqual(bad.status_code,400)
            bad=await self.client.post(endpoint+'/browser-login',data={**form,'password':'wrong-password-long'},headers=headers)
            ...
            self.assertIn('Path=/',cookie);self.assertNotIn('Domain=',cookie)
            ...
            self.assertEqual((await self.client.get(endpoint+'/me')).status_code,401)  # Cookie cannot bypass Bearer auth.
            ...
            rejected=await self.client.post(endpoint+'/browser-logout',data={'return_to':form['return_to']},headers={'Origin':'https://attacker.invalid'})
            ...
            result=await self.client.post(endpoint+'/browser-logout',data={'return_to':form['return_to']},headers=headers)
            self.assertEqual(result.status_code,200)

    async def test_browser_cookie_secure_over_https(self):
        with patch.object(settings,'CORS_ORIGINS','https://career.example'),patch.object(settings,'AUTH_COOKIE_SECURE',True):
            result=await self.client.post('https://career.example/api/auth/browser-login',
                data={'username':'alice','password':'test-password-long','return_to':'https://career.example/'},
    ...
    async def test_foreign_resources_denied(self):
        ...
        self.assertEqual({r['id'] for r in (await self.client.get('/api/documents',headers=self.ah)).json()},{str(da)})
        self.assertEqual({r['run_id'] for r in (await self.client.get('/api/runs',headers=self.ah)).json()},{str(ownrun)})
        self.assertEqual({r['id'] for r in (await self.client.get('/api/chat/conversations',headers=self.ah)).json()},{str(ca)})
        cases=[('GET',f'/api/runs/{run}',None),('DELETE',f'/api/documents/{db}',None),
               ('GET',f'/api/chat/conversations/{cb}/messages',None),('PATCH',f'/api/chat/conversations/{cb}?title=stolen',None),
               ...
               ('POST','/api/match/resume',{'thread_id':str(run)}),('POST','/api/match/stream/resume',{'thread_id':str(run)}),
               ('POST','/api/interview',{'match_run_id':str(run)}),('POST','/api/learning-plan',{'match_run_id':str(run)})]
        for method,path,body in cases:
            response=await self.client.request(method,path,headers=self.ah,json=body)
            self.assertEqual(response.status_code,404,(path,response.text));await self.reset_counters()
        response=await self.client.post('/api/match/resume',headers=self.ah,json={'thread_id':str(run),'run_id':str(ownrun)})
    ...
    async def test_search_ownership_filters(self):
        ...
        for path,service in [('/api/search',api.milvus_service),('/api/search/bm25',api.bm25_service),('/api/search/hybrid',api.hybrid_search_service)]:
            search=AsyncMock(return_value=[mine,theirs])
            with patch.object(service,'search',search),patch.object(api.bailian_service,'embed_query',AsyncMock(return_value=[0])):
            ...
            self.assertEqual(search.call_args.kwargs['allowed_document_ids'],[str(da)]);self.assertNotIn('secret',response.text)
    async def test_inputs_and_body_caps(self):
        for body in [{'message':'a'*(settings.CHAT_MAX_INPUT_CHARS+1)},{'message':'hi','user_id':str(self.bob)},{'message':' '}]:
            response=await self.client.post('/api/chat/completions',headers=self.ah,json=body);self.assertEqual(response.status_code,422,response.text)
        response=await self.client.post('/api/chat/completions',headers=self.ah,content=b'x'*(settings.JSON_BODY_MAX_BYTES+1))
    ...
    async def test_atomic_rate_limit_concurrent(self):
        with patch.object(settings,'GENERATION_REQUESTS_PER_MINUTE',2):
            responses=await asyncio.gather(*[self.client.post('/api/search/bm25',headers=self.ah,json={'query':'q'}) for _ in range(5)])
    ...
    async def test_conversation_lock_and_operation_limit(self):
        *_,ca,cb=await self.seed();started=asyncio.Event();finish=asyncio.Event()
        async def chat(**kwargs):started.set();await finish.wait();return 'answer'
        with patch.object(chat_api.bailian_service,'chat',chat):
            task=asyncio.create_task(self.client.post('/api/chat/completions',headers=self.ah,json={'conversation_id':str(ca),'message':'q','stream':False}))
            await asyncio.wait_for(started.wait(),2)
            response=await self.client.post('/api/chat/completions',headers=self.ah,json={'conversation_id':str(ca),'message':'q2','stream':False})
            ...
            finish.set();self.assertEqual((await task).status_code,200)
        async with self.engine.connect() as connection:
            key='chat:'+str(ca)
            self.assertTrue((await connection.execute(text('SELECT pg_try_advisory_lock(hashtextextended(:k,0))'),{'k':key})).scalar())
    ...
    async def test_timeout_cleans_stream_and_leases(self):
        ...
        async def stream(**kwargs):
            try:yield 'partial';await asyncio.sleep(10)
            finally:stopped.set()
        with patch.object(settings,'CHAT_TOTAL_TIMEOUT',.25),patch.object(chat_api.bailian_service,'stream_messages',stream):
            response=await self.client.post('/api/chat/completions',headers=self.ah,json={'message':'q'})
        self.assertEqual(response.status_code,200);self.assertIn('总耗时',response.text);self.assertIn('[DONE]',response.text);self.assertTrue(stopped.is_set())
        ...
        async with self.factory() as session:
            ...
            self.assertTrue(rows);self.assertTrue(rows[0].meta_data['incomplete'])
    async def test_budget_error_reaches_sse_client(self):
        ...
        with patch.object(chat_api.bailian_service,'stream_messages',stream):
            response=await self.client.post('/api/chat/completions',headers=self.ah,json={'message':'q'})
        self.assertEqual(response.status_code,200);self.assertIn('调用次数',response.text);self.assertIn('[DONE]',response.text)
    async def test_upload_deduplication_is_per_owner(self):
        ...
        with patch.object(module.minio_service,'upload',upload), \
             patch.object(module.bailian_service,'embed',AsyncMock(return_value=[[0.0]*settings.EMBEDDING_DIMENSION])), \
            ...
            for headers in (self.ah,self.ah,self.bh):
                response=await self.client.post('/api/documents/upload',headers=headers,
                    files={'file':('test.txt',b'Python experience','text/plain')},data={'document_type':'resume'})
                self.assertEqual(response.status_code,200,response.text);results.append(response.json())
    ...
    async def test_workflow_stream_timeout_stops_and_marks_run_failed(self):
        stopped=asyncio.Event()
        async with self.factory() as session:
            document=Document(owner_id=self.alice,filename='resume.txt',document_type='resume',status='ready',minio_object_key='local')
            session.add(document);await session.flush();identifier=document.id
            session.add(DocumentParse(document_id=identifier,pipeline_version='test',text='Python developer',blocks=[],chunks=[]))
            await session.commit()
        ...
        with patch.object(settings,'MATCH_TOTAL_TIMEOUT',2),patch.object(api,'run_match_stream',workflow):
            response=await self.client.post('/api/match/stream',headers=self.ah,json={'resume_document_id':str(identifier),'jd_text':'Python'})
        self.assertEqual(response.status_code,200,response.text);self.assertIn('总耗时',response.text)
        self.assertTrue(stopped.is_set())
        async with self.factory() as session:
            runs=(await session.execute(select(AgentRun))).scalars().all()
            self.assertEqual(len(runs),1);self.assertEqual(runs[0].status,'failed')
            self.assertEqual((await session.execute(select(func.count()).select_from(OperationLease))).scalar(),0)
