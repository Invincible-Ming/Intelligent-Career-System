...
def evaluation(education=80):
    return MatchEvaluation(scores=MatchScores(skill=80,experience=75,responsibility=70,education=education,bonus=10),
...
def state():
    ...
    return dict(resume_analysis=resume,job_analysis=jd,match_report=report,result=report,
                human_reviewed=True,research_completed=False,human_feedback='',verification_feedback='',retry_count=0)
...
class ConfirmationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_timeout_reuses_report_and_is_not_repeated(self):
        ...
        model.assert_not_called();tool.ainvoke.assert_awaited_once()
        self.assertTrue(canceled.is_set())
        self.assertTrue(result['research_completed'])

    async def test_invalid_agent_message_is_not_evidence(self):
        tool=SimpleNamespace(name='search_web',ainvoke=AsyncMock(return_value='Sorry, need more steps to process this request.'))
        with patch.object(workflow.mcp_service,'get_tools',return_value=[tool]), \
             patch.object(workflow.bailian_service,'structured_chat') as model:
            result=await workflow.match_agent(state(),{'configurable':{}})
        model.assert_not_called();self.assertEqual(result['research_context'],'')
    ...
    async def test_feedback_regenerates_but_never_repeats_completed_search(self):
        ...
        with patch.object(workflow.mcp_service,'get_tools') as tools, \
            ...
            result=await workflow.match_agent(original,{'configurable':{}})
        tools.assert_not_called();model.assert_awaited_once()
    ...
    async def test_review_applies_correction_and_recalculates_total_once(self):
        original=state()
        review=VerificationResult(is_faithful=False,reason='Synthetic graduation mismatch',corrected_evaluation=evaluation(20))
        ...
        self.assertTrue(result['verification_corrected']);self.assertTrue(result['is_valid'])
    ...
    async def test_uncorrectable_review_marks_risk_and_stops_generation_loop(self):
        ...
        self.assertFalse(result['is_valid']);self.assertTrue(result['is_degraded'])
    ...
    async def test_real_graph_confirmation_uses_one_review_call_with_correction(self):
        ...
        async def model(**kwargs):
            schema=kwargs['response_model'];schemas.append(schema)
            ...
            if schema is VerificationResult:return VerificationResult(is_faithful=False,reason='Synthetic issue',corrected_evaluation=evaluation(20))
        ...
        with patch.object(workflow,'match_workflow',graph),patch.object(settings,'ANALYSIS_CACHE_ENABLED',False), \
            ...
            initial=[event async for event in workflow.run_match_stream(resume_text='Synthetic resume',jd_text='Synthetic JD',owner_id='synthetic-owner')]
            tools.assert_not_called()
            thread=json.loads(initial[-1].splitlines()[1].split(': ',1)[1])['thread_id']
            schemas.clear()
            resumed=[event async for event in workflow.resume_match_stream(thread_id=thread,owner_id=str(uuid.uuid4()))]
    ...
    async def test_confirmed_search_precedes_update_and_review_in_nonstream_graph(self):
        ...
        async def search(args, **kwargs):
            ...
            return {'results':[{'title':'Synthetic job','url':'https://example.com/job','snippet':'Synthetic role evidence'}]}
        tool=SimpleNamespace(name='search_web',ainvoke=AsyncMock(side_effect=search))
        async def model(**kwargs):
        ...
        with patch.object(workflow,'match_workflow',graph),patch.object(settings,'ANALYSIS_CACHE_ENABLED',False), \
             ...
             patch.object(workflow.bailian_service,'structured_chat',side_effect=model):
            initial=await workflow.run_match(resume_text='Synthetic resume',jd_text='Synthetic JD',owner_id='synthetic-owner')
            tools.assert_not_called()
            trace.clear()
