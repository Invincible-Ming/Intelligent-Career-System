...
if config.exists():
    password = next(line.split(" ", 1)[1] for line in config.read_text().splitlines() if line.startswith("requirepass "))
else:
...
contents = re.sub(r"^REDIS_URL=.*$", lambda _: line, contents, flags=re.MULTILINE) if re.search(r"^REDIS_URL=", contents, re.MULTILINE) else contents.rstrip() + "\n" + line + "\n"
