...
async def purge_legacy():
    ...
    async with AsyncSessionLocal() as session:
        owned_ids = set(map(str, (await session.execute(select(Document.id).where(Document.owner_id.is_not(None)))).scalars()))
        legacy_documents = (await session.execute(select(Document.id, Document.minio_object_key).where(Document.owner_id.is_(None)))).all()
        legacy_runs = list((await session.execute(select(AgentRun.id).where(AgentRun.owner_id.is_(None)))).scalars())
        legacy_conversations = list((await session.execute(select(Conversation.id).where(Conversation.owner_id.is_(None)))).scalars())
    ...
    objects = await asyncio.to_thread(lambda: list(minio_service.client.list_objects(minio_service.bucket, prefix='documents/', recursive=True)))
    ...
    async with engine.begin() as connection:
        ...
        owned_threads = list(map(str, (await connection.execute(select(AgentRun.id).where(AgentRun.owner_id.is_not(None)))).scalars()))
...
async def main():
    ...
    parser.add_argument('--delete-unowned', action='store_true', help='Destructive; requires explicit operator authorization')
