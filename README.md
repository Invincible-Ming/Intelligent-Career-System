...
基于 LangGraph Multi-Agent 与 RAG 的智能求职辅助平台：简历与岗位多维匹配、RAG 智能对话、面试真题预测、定制学习计划，并内置管理员专属的 RAG 评测模块。

## 功能总览

| 页面 | 说明 |
|------|------|
| 智能对话 | 基于 RAG 的求职问答助手，混合检索 + 流式回答，会话支持置顶 / 重命名 / 跨设备续聊 |
| 知识库 | 文档上传（PDF / DOCX / XLSX / TXT）、解析、切片、向量化入库；多路混合检索验证 |
...
安全沙箱：沙箱 Runner（持有 Docker 权限，Unix socket 通信）
        └── 临时容器：文档解析 / MCP 工具（无网络、只读、用后即删）
```

- 所有模型调用（对话 qwen3.8-flash、向量化 text-embedding、OCR qwen-vl-ocr）统一由后端执行，密钥不进入沙箱容器
- 文档解析在一次性容器中完成：无网络、只读根文件系统、资源受限，解析完成即销毁
...
复制环境变量模板并填写（数据库、Redis、MinIO、Milvus、百炼 API Key 等）：
...
| `MAX_MODEL_CALLS_PER_OPERATION` | 单次操作模型调用上限 | 200 |
