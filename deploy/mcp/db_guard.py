...
ACCESS_QUERY = """
    ...
    AND (has_table_privilege(current_user, c.oid, 'SELECT,INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER')
         OR has_any_column_privilege(current_user, c.oid, 'SELECT,INSERT,UPDATE,REFERENCES')))               AS extra_access,
                      EXISTS (SELECT 1
                              FROM pg_auth_members m
                                       JOIN pg_roles r ON r.oid = m.member
                              WHERE r.rolname = current_user)                                                AS membership,
                      EXISTS (SELECT 1
                              FROM pg_namespace
                              WHERE nspname !~ '^pg_' AND has_schema_privilege(current_user, oid, 'CREATE')) AS schema_create,
                      has_database_privilege(current_user, current_database(), 'CREATE')                     AS database_create \
               """


def check_identity(role, access, readonly):
    if role["rolname"] != DATABASE_ROLE or any(
            role[k] for k in ("rolsuper", "rolcreaterole", "rolcreatedb", "rolreplication", "rolbypassrls")):
